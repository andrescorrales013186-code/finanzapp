...
// ThemeContext.jsx
//
// CAMBIO PRINCIPAL: se agrega "accentText" a cada tema.
// accentText = color del TEXTO que va ENCIMA del color de acento (a500).
// Si el acento es claro → accentText es oscuro (#1a0a00)
// Si el acento es oscuro → accentText es blanco (#ffffff)
//
// Esto resuelve el problema de letras invisibles en temas claros como
// GitHub Light o Solarized Light donde el acento era azul oscuro sobre
// fondo blanco — la letra quedaba ilegible.
// ─────────────────────────────────────────────────────────────────────────────
...
export function ThemeProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const vars = {
      '--accent-50':   theme.a50,
      '--accent-100':  theme.a100,
      '--accent-200':  theme.a200,
      '--accent-400':  theme.a400,
      '--accent-500':  theme.a500,
      '--accent-600':  theme.a600,
      '--accent-700':  theme.a700,
      '--accent-800':  theme.a800,
      '--accent-950':  theme.a950,
      '--accent-text': theme.accentText,  // ← NUEVO: texto sobre el acento
